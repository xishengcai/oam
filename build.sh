...
if [ -z "$VERSION" ]; then
  VERSION='v100r001c02b071'
fi

if [ -z "$IMAGE_REPO" ]; then
#  IMAGE_REPO='registry.cn-beijing.aliyuncs.com/launcher-agent-only'
  IMAGE_REPO='registry.cn-hangzhou.aliyuncs.com/launcher-agent-only'
fi
